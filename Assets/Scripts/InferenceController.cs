...
public class InferenceController : MonoBehaviour
{
    ...
    [Header("ByteTrack")]
    [SerializeField, Tooltip("The frame rate for the ByteTrack object tracker")]
    private int frame_rate = 200;
    ...
    public void Start()
    {
        ByteTrackPlugin.init_tracker(frame_rate: frame_rate);
    }
    ...
    private void Update()
    {
        ...
        float[] boxes_probs_array = new float[bboxInfoArray.Length * 5];

        for (int i = 0; i < bboxInfoArray.Length; i++)
        {
            boxes_probs_array[i * 5] = bboxInfoArray[i].bbox.x0;
            boxes_probs_array[i * 5 + 1] = bboxInfoArray[i].bbox.y0;
            boxes_probs_array[i * 5 + 2] = bboxInfoArray[i].bbox.width;
            boxes_probs_array[i * 5 + 3] = bboxInfoArray[i].bbox.height;
            boxes_probs_array[i * 5 + 4] = bboxInfoArray[i].bbox.prob;
        }

        int[] track_ids_array = new int[bboxInfoArray.Length];
        Array.Fill(track_ids_array, -1);
        ByteTrackPlugin.get_track_ids(boxes_probs_array, bboxInfoArray.Length, track_ids_array);
        //Debug.Log($"track_ids: {string.Join(" ", track_ids_array)}");

        List<BBox2DInfo> filteredBboxInfoArray = new List<BBox2DInfo>();

        for (int i = 0; i < track_ids_array.Length; i++)
        {
            if (track_ids_array[i] != -1)
            {
                BBox2DInfo bboxInfo = new BBox2DInfo();
                bboxInfo.bbox = bboxInfoArray[i].bbox;
                bboxInfo.color = bboxInfoArray[i].color;
                bboxInfo.label = $"{track_ids_array[i]}-{bboxInfoArray[i].label}";
                filteredBboxInfoArray.Add(bboxInfo);
            }
        }

        uiController.UpdateUI(filteredBboxInfoArray.Count);
        boundingBoxVisualizer.UpdateBoundingBoxVisualizations(filteredBboxInfoArray.ToArray());
    }
